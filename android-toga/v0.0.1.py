...
class MyApp(toga.App):
    ...
    def startup(self):
        global Cpu_speed
        global do_you_save, ping_range_see , timeout_see , count_see , interval_see
        selff=self
        ...
        def check_ip6_again():
                ...
                label_ipv4.style.update(background_color=color1)
                label_ipv6.text=f"ipv6 : {check[1]}"
                label_ipv6.style.update(background_color=color2)
        ...
        def change(rer):
            ...
            with open(file_path , "r") as f:
                saved=f.readlines()
            saved[0]=f_s.value+"\n"
            saved[1]=save_reys.value+"\n"
            saved[2]=wich.value+"\n"
            saved[3]=with_port.value+"\n"
            saved[4]=ping_range.value+"\n"
            saved[5]=timeout.value+"\n"
            saved[6]=count.value+"\n"
            saved[7]=interval.value+"\n"
...
'''
            ...
            "stats": {}
                ...
                def check_ac_v6(mn):
                    ...
                    for ip, port, ping, loss_rate,jitter , combined_score in sorted_results[:10]:
                        table.data.append({"ip": ip, "ping": ping,"lost":loss_rate,"jitter":jitter,"score": combined_score})
                    ...
                    label_best.value= f"The best IP: [{ip}]:{port if port else 'N/A'} , ping: {ping:.2f} ms, packet loss: {loss_rate:.2f}% ,{jitter} ms , score: {combined_score:.2f}" 
            
                    best_result=2*[1]
                    best_result[0]=f"{ip}"
                    best_result[1]=878
                    labelmain1.text= "Finished"
                ...
                # "IP", "Ping", "lost", "jitter", "score"
                    ...
                    try:
                            ...
                            label_best.value=f"The best IP: {ip}:{port if port else '878'} , ping: None, packet loss: {loss_rate:.2f}%, jitter: {jitter} ms , score: {combined_score:.2f}"
        ...
        'Address': bi[0]
                                    ...
                                    label_best.value=generate_wireguard_url(bi,str(ip)+":"+str(port))
                                go()
        ...
        'Address': bi[0]
                                ...
                                label_best.value=wow_nonoise("["+ip+"]",port)
                                ...
                                return
                    if do_you_save =="yes\n":
                        if os.path.exists("sdcard/Download/wwarpscanner/result.txt"):
                            try:
                                os.remove("sdcard/Download/wwarpscanner/result.txt")
                            except Exception:
                                pass
        ...
        container = toga.OptionContainer(style=Pack(width=width),
            content=[
        ...
        ch_ag = toga.Button('Check', style=Pack(padding=(0, 580, 0 , 0), width=100, color='#848484')) # top, right, bottom, left
        ...
        radio_var = toga.Selection(items=['IP scanning', 'Get config'], on_select=on_select, style=Pack(padding=(0, 5)))
        ...
        wich_loc = toga.Selection(items=['Iran', 'German'], style=Pack(padding=(0, 5)))
        ...
        button_clean = toga.Button('clean', on_press=clean, style=Pack(padding=(0, 5)))
        ip_box.add(button_clean)
        file_path ="sdcard/Download/wwarpscanner/warp_setting"
        with open(file_path , "r") as f:
            saved=f.readlines()
