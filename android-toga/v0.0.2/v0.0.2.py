...
class MyApp(toga.App):
    ...
    def startup(self):
        ...
        def clean2(elr):
            ...
            label_best.value="saved in Download/clean_result.txt !'"         
            ...
            "stats": {}
                ...
                def check_ac_v6(mn):
                    ...
                    for ip, port, ping, loss_rate,jitter , combined_score in sorted_results[:10]:
                         ...
                         q.put(lambda ip=ip, port=port, ping=ping, loss_rate=loss_rate, jitter=jitter, combined_score=combined_score:setattr(table, 'data', {"ip": ip, "ping": ping, "lost": loss_rate, "jitter": jitter, "score": combined_score}))
