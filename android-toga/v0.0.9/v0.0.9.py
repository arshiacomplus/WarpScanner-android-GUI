...
file_path = "/storage/emulated/0/Download/wwarpscanner/warp_setting"

error=False
try:
    with open(file_path, "r") as f:
        num = f.readlines()
        Cpu_speed = num[0].strip()
        do_you_save = num[1].strip()
        wich_pannel = num[2].strip()
        port_go = num[3].strip()
        ping_range_see = int(num[4].strip())
        timeout_see = int(num[5].strip())
        count_see = int(num[6].strip())
        interval_see = int(num[7].strip())
...
class MyApp(toga.App):
    ...
    def startup(self):
        home_icon=self.paths.app / "resources" / "home.png"
        table_icon=self.paths.app / "resources" / "table.png"
        wheele_icon=self.paths.app / "resources" / "setting.png"
        ...
        global Cpu_speed
        global do_you_save, ping_range_see , timeout_see , count_see , interval_see,wich_pannel,port_go,error
        ...
        def clean2(elr):
            with open("/storage/emulated/0/Download/wwarpscanner/result.txt", 'r') as f:
                    b=f.readlines()
            ...
            label_best.value="saved in /storage/emulated/0/Download/clean_result.txt !'"         
        ...
        def theard_check_ip6_again(self):
            def check_ip6_again(vfgh):
                            ...
                            if  check[1] == "Available":
                                ...
                                color2="red"
                            q.put(lambda: setattr(label_ipv4, 'text', f"ipv4 : {check[0]}")) 
                            q.put(lambda color=color1:label_ipv4.style.update(color=color1)) 
                            q.put(lambda: setattr(label_ipv6, 'text', f"ipv6 : {check[1]}")) 
                    
                            q.put(lambda color=color1:label_ipv6.style.update(color=color2)) 
            ...
            thread3=threading.Thread(target=check_ip6_again, args=(stop_event,))
            thread3.daemon=True
            thread3.start()
        ...
        def change(rer):
            ...
            with open(file_path , "w") as f :
                for k in saved:
                    f.write(k)
            Cpu_speed=f_s.value.strip()
            do_you_save=save_reys.value.strip()
            wich_pannel=wich.value.strip()
            port_go=with_port.value.strip()
                        
            ping_range_see=int(ping_range.value.strip())
            timeout_see=int(timeout.value.strip())
            count_see=int(count.value.strip())
            interval_see=int(interval.value.strip())
        ...
        def issub(widget):
            ...
            if val=="wireguard for Hiddify without ip scanning" or val=="wireguard for Sing-box and Hiddify| old | without ip scanning" or val=="wireguard for v2ray and mahsaNG without ip scanning" or val=="wireguard for nikaNg and mahsaNg without ip scanning":
                i_ip_scan=True
                ip_box.remove(button_ipv4)
                ip_box.remove(button_ipv6)
                ip_box.remove(button_clean)
        ...
        def update_label( ip, port, ping, loss_rate, jitter, combined_score):
            label_best.value = f"The best IP: {ip}:{port if port else 'N/A'} , ping: {ping:.2f} ms, packet loss: {loss_rate:.2f}%, jitter: {jitter} ms , score: {combined_score:.2f}"
        def main_v4():
            ...
            def main_menu():
                ...
                if Cpu_speed == "faster": max_workers_number=100
                elif Cpu_speed == "slower": max_workers_number=50
                print(Cpu_speed)
                ...
                def get_wireguard_for_hiddify(best_ip,num):
                    ...
                    if optionmenu.value=="wireguard for Hiddify" or optionmenu.value=="wireguard for Hiddify without ip scanning" or optionmenu.value=="wireguard for Sing-box and Hiddify| old | without ip scanning":
                        ...
                        q.put(lambda: setattr(label_best, 'value', f'''{{
  "outbounds": 
  [{hising}
  ]
}}
...
'''
                ...
                def generate_wireguard_url(config, endpoint):
                    ...
                    if optionmenu.value=="wireguard for nikaNg and mahsaNg" or optionmenu.value=="wireguard for nikaNg and mahsaNg without ip scanning":
                                wireguard_urll = (
                    f"wireguard://{urlencode(config['PrivateKey'])}@{endpoint}"
            ...
            "stats": {{}}
                ...
                def check_ac_v6(mn):
                    ...
                    self.loop.call_soon_threadsafe(update_label, ip, port, ping, loss_rate, jitter, combined_score)
                    time.sleep(5)
                ...
                if i_ip_scan==True:
                    ...
                    if optionmenu.value=="wireguard for Hiddify without ip scanning" or optionmenu.value=="wireguard for Sing-box and Hiddify| old | without ip scanning":
                        ...
                        get_wireguard_for_hiddify((ip,int(port),0,0,0,0),1)
                       

                    elif optionmenu.value=="wireguard for v2ray and mahsaNG without ip scanning" or optionmenu.value=="wireguard for nikaNg and mahsaNg without ip scanning":
                                print("w_no")
                                ip=no_ip.value
                                port=no_port.value
                                def go():
                    ...
                    if radio_var.value=="Get config":
                            ...
                            if optionmenu.value=="wireguard for Hiddify with a sub link"  or optionmenu.value=="wireguard for Sing-box and Hiddify | old | with a sub link":
                                ...
                                for i in range(int(how_menypp.value)):
                                        ...
                                        get_wireguard_for_hiddify(sorted_results[0],i)
                                ...
                                get_wireguard_for_hiddify(sorted_results[0],1)
                               
                                return
                            elif optionmenu.value=="wireguard for v2ray and mahsaNG" or optionmenu.value=="wireguard for nikaNg and mahsaNg":
                                def go():
                                    bi=bind_keys()
                                    bi={
        'PrivateKey': bi[1],
        ...
        'Address': bi[0]
                    ...
                    if do_you_save =="yes":
                        ...
                        with open("/storage/emulated/0/Download/wwarpscanner/result.txt" ,"w") as f:
                            for i in save_result:
                                f.write(i)                                                                     
        ...
        main_box = toga.Box(style=Pack(alignment=LEFT,flex=1,direction=ROW,padding=(5)))
      
    
        ip_box= toga.Box(style=Pack(alignment=LEFT,color="gray", direction=COLUMN,padding=(5),flex=1))
        
        table_box= toga.Box(style=Pack(alignment=LEFT,direction=COLUMN,padding=(5),flex=1))

        set_box= toga.Box(style=Pack(alignment=LEFT, direction=COLUMN,padding=(5),flex=1))
        scrollable_box = toga.ScrollContainer(content=set_box,style=Pack(alignment=LEFT,flex=1, direction=COLUMN), horizontal=False, vertical=True)
        scrollable_box2 = toga.ScrollContainer(content=table_box,style=Pack(alignment=LEFT,flex=1, direction=COLUMN), horizontal=False, vertical=True)
        scrollable_box3 = toga.ScrollContainer(content=ip_box,style=Pack(alignment=LEFT,flex=1, direction=COLUMN), horizontal=False, vertical=True)

        self.main_window = toga.MainWindow(title=self.formal_name)
        width=self.main_window.screen.size.width
  
        
        container = toga.OptionContainer(style=Pack(alignment=LEFT,flex=1,font_size=14),
            content=[
            toga.OptionItem("main ", scrollable_box3, icon=toga.Icon(home_icon)),
            toga.OptionItem("table", scrollable_box2, icon=toga.Icon(table_icon)),
            toga.OptionItem("setting", scrollable_box , icon=toga.Icon(wheele_icon))
            ]
        )
        ...
        if error==True:
            errro_la=toga.Label('please delete Download/wwarpscanner ! ', style=Pack(alignment=CENTER,padding=(0, 5),color='red'))
            ip_box.add(errro_la)
        
           
            toga.InfoDialog("Error", "First :restart the  app if not work please delete this folder Download/wwarpscannr")
            self.main_window.content = main_box
            self.main_window.show()
            return
   
        label_ipv4 = toga.Label('ipv4 : none', style=Pack(alignment=LEFT,padding=(0, 5)))
        ip_box.add(label_ipv4)

        label_ipv6 = toga.Label('ipv6 : none', style=Pack(alignment=LEFT,padding=(0, 5)))
        ip_box.add(label_ipv6)

        ch_ag = toga.Button('Check', style=Pack(alignment=LEFT,padding=(0, 580, 0 , 0)
        ...
        labelmain1 = toga.Label('Click to scan ip', style=Pack(alignment=LEFT,padding=(25, 0,5,0,)))
        ip_box.add(labelmain1)

        label_best = toga.MultilineTextInput(style=Pack(alignment=LEFT,padding=(10, 20, 10, 20)),placeholder="result:")

        ip_box.add(label_best)

        how_menypp = toga.TextInput(placeholder='how many? (2~4)', style=Pack(alignment=LEFT,padding=(0, 5)))

        no_ip = toga.TextInput(placeholder='ipv4 or ipv6', style=Pack(alignment=LEFT,padding=(3,0,0, 5)))
        
        no_port = toga.TextInput(placeholder='port', style=Pack(alignment=LEFT,padding=(3,0,0, 5)))

        radio_var = toga.Selection(items=['IP scanning', 'Get config'], on_select=on_select, style=Pack(alignment=LEFT,padding=(5, 5)))
        ...
        "wireguard for nikaNg and mahsaNg without ip scanning",
        "WoW with noise for Nikang or MahsaNg",
        "WoW with noise for Nikang or MahsaNg in sub link",
        "wireguard for Sing-box and Hiddify | old |",
        "wireguard for Sing-box and Hiddify| old | without ip scanning",
        "wireguard for Sing-box and Hiddify | old | with a sub link",
        ], style=Pack(alignment=LEFT,padding=(5, 5)))

        wich_loc = toga.Selection(items=['Iran', 'German'], style=Pack(alignment=LEFT,padding=(2, 5)))



        button_ipv4 = toga.Button('ipV4', on_press=main_move, style=Pack(alignment=LEFT,padding=(0, 5)))
        ip_box.add(button_ipv4)

        button_ipv6 = toga.Button('ipV6', on_press=main_move_v6, style=Pack(alignment=LEFT,padding=(0, 5)))
        ip_box.add(button_ipv6)

        button_clean = toga.Button('clean', on_press=clean2, style=Pack(alignment=LEFT,padding=(0, 5)))
        ip_box.add(button_clean)
        file_path ="/storage/emulated/0/Download/wwarpscanner/warp_setting"
        with open(file_path , "r") as f:
            saved=f.readlines()
        power = toga.Label('Scanpower', style=Pack(alignment=LEFT,padding=(25, 0,5,0)))
        set_box.add(power)

        f_s = toga.Selection(items=['slower', 'faster'], value=saved[0][:len(saved[0])-1], on_select=change, style=Pack(alignment=LEFT,padding=(0, 5)))
        set_box.add(f_s)

        save_re = toga.Label('save result', style=Pack(alignment=LEFT,padding=(25, 0,5,0)))
        set_box.add(save_re)

        save_reys = toga.Selection(items=['yes', 'no'], value=saved[1][:len(saved[1])-1], on_select=change, style=Pack(alignment=LEFT,padding=(0, 5)))
        set_box.add(save_reys)

        wich_la = toga.Label('whcih pannel', style=Pack(alignment=LEFT,padding=(25, 0,5,0)))
        set_box.add(wich_la)

        wich = toga.Selection(items=['bpb', 'vahid', "with score"], value=saved[2][:len(saved[2])-1], on_select=change, style=Pack(alignment=LEFT,padding=(0, 5)))
        set_box.add(wich)

        with_port_la = toga.Label('with port', style=Pack(alignment=LEFT,padding=(25, 0,5,0)))
        set_box.add(with_port_la)

        with_port = toga.Selection(items=['yes', 'no'], value=saved[3][:len(saved[3])-1], on_select=change, style=Pack(alignment=LEFT,padding=(0, 5)))
        set_box.add(with_port)


        ping_range_la = toga.Label('ping_range', style=Pack(alignment=LEFT,padding=(25, 0,5,0)))
        set_box.add(ping_range_la)

        ping_range = toga.TextInput(style=Pack(alignment=LEFT,padding=(25, 0,5,0)),value=saved[4][:len(saved[4])-1],on_lose_focus=change,placeholder="from zero to what?: ")        
        set_box.add(ping_range)



        timeout_la = toga.Label('icmp_timeout', style=Pack(alignment=LEFT,padding=(25, 0,5,0)))
        set_box.add(timeout_la)

        timeout = toga.TextInput(style=Pack(alignment=LEFT,padding=(25, 0,5,0)),value=saved[5][:len(saved[5])-1],on_lose_focus=change,placeholder="1~5")        
        set_box.add(timeout)

        count_la = toga.Label('icmp_count', style=Pack(alignment=LEFT,padding=(25, 0,5,0)))
        set_box.add(count_la)

        count = toga.TextInput(style=Pack(alignment=LEFT,padding=(25, 0,5,0)),value=saved[6][:len(saved[6])-1],on_lose_focus=change,placeholder="1~5")        
        set_box.add(count)

        interval_la = toga.Label('icmp_interval', style=Pack(alignment=LEFT,padding=(25, 0,5,0)))
        set_box.add(interval_la)

        interval = toga.TextInput(style=Pack(alignment=LEFT,padding=(25, 0,5,0)),value=saved[7][:len(saved[7])-1],on_lose_focus=change,placeholder="1~5")        
        set_box.add(interval)


        table = toga.Table(style=Pack(alignment=LEFT,flex=1),
